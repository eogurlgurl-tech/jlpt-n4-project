...
let progress = JSON.parse(localStorage.getItem(STORAGE_KEY)) || defaultData;
...
function getQuizStats() {
  return (
    JSON.parse(localStorage.getItem(QUIZ_STATS_KEY)) || {
      total: 0,
      correct: 0,
      wrong: 0,
    }
...
function renderTodayStudy() {
  ...
  const target = document.getElementById("todayStudyArea");
...
function toggleReading() {
  const box = document.getElementById("grammarReading");
...
document.getElementById("startBtn").addEventListener("click", () => {
  ...
  alert(`DAY ${progress.currentDay} 완료!`);
});
...
document.querySelectorAll(".tab-btn").forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    if (tab === "favorite") {
      ...
      if (typeof renderFavoriteTab === "function") {
        renderFavoriteTab();
      }
    }
    ...
    if (tab === "achievement") {
      ...
      if (typeof renderAchievementTab === "function") {
    ...
    if (tab === "weakness") {
      const weaknessTab = document.getElementById("weaknessTab");

      weaknessTab.style.display = "block";

      if (typeof renderWeaknessTab === "function") {
        renderWeaknessTab();
      }
    }
    ...
    if (tab === "mockexam") {
      ...
      mockexamTab.style.display = "block";
...
document.getElementById("grammarPrev").addEventListener("click", () => {
  grammarIndex--;

  if (grammarIndex < 0) {
    grammarIndex = GRAMMAR_DATA.length - 1;
  }
...
document.getElementById("grammarNext").addEventListener("click", () => {
  grammarIndex++;
...
document.getElementById("grammarRandom").addEventListener("click", () => {
  grammarIndex = Math.floor(Math.random() * GRAMMAR_DATA.length);
...
document.getElementById("vocabPrev").addEventListener("click", () => {
  ...
  if (vocabIndex < 0) {
    vocabIndex = VOCAB_DATA.length - 1;
  }
...
document.getElementById("vocabRandom").addEventListener("click", () => {
  vocabIndex = Math.floor(Math.random() * VOCAB_DATA.length);
...
document.getElementById("kanjiPrev").addEventListener("click", () => {
  ...
  if (kanjiIndex < 0) {
    kanjiIndex = KANJI_DATA.length - 1;
  }
...
document.getElementById("kanjiRandom").addEventListener("click", () => {
  kanjiIndex = Math.floor(Math.random() * KANJI_DATA.length);
